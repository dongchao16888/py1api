...
def getLogHandler(log_dir):
    ...
    '''
    log_dir = "{}/{}".format(log_dir, log_file_name)
    # 默认日志等级的设置
    logging.basicConfig(level=logging.DEBUG)
    # 创建日志记录器，指明日志保存路径,每个日志的大小，保存日志的上限
    file_log_handler = RotatingFileHandler(log_dir, maxBytes=1024 * 1024, backupCount=10, encoding='UTF-8')
